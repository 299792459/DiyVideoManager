...
| `templates/index.html` | 单页：视频 / **标签管理** / 设置 多视图切换。 |
...
- **列表 API**：`GET /api/videos` 每条含 **`tags`**（名称数组）与 **`tag_items`**（`{id,name}`），供前端**逐条改/删**标签。
...
2. 读本文 + **`docs/CRAWLER_README.md`**（若动爬虫）。  
3. 改 UI 时同步看 **`static/style.css`**。  
4. 新增 HTTP 接口时：保持与现有 JSON 风格一致；敏感配置不要写进仓库。
